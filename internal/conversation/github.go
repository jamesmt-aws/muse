...
type GitHub struct {
	...
	MinContentWords int // minimum total words from owner per conversation (0 = default 2-message filter)
}
...
func (g *GitHub) Conversations(ctx context.Context, progress func(SyncProgress)) ([]Conversation, error) {
	...
	for _, t := range threads {
		...
		conv := assembleCachedConversation(username, g.source, t, g.MinContentWords)
		if conv != nil {
...
func assembleCachedConversation(username, source string, t cachedThread, minContentWords int) *Conversation {
	...
	conv := assembleGitHubConversation(username, source, t.Owner, t.Repo, t.Number, t.IsPR, t.Title, t.CreatedAt, t.UpdatedAt, messages, minContentWords)
...
func assembleGitHubConversation(username, source, owner, repo string, number int, isPR bool, title string, createdAt, updatedAt time.Time, messages []githubMessage, minContentWords int) *Conversation {
	...
	for _, m := range messages {
		if strings.EqualFold(m.Author, username) {
			ownerCount++
			ownerWords += len(strings.Fields(m.Body))
		}
	}
	if minContentWords > 0 {
		if ownerWords < minContentWords {
			return nil
		}
